...
class DataLoader:
    ...
    def load_training_data(self, version: str = "v0.2") -> pd.DataFrame:
        ...
        self.training_data["Timestamp_Local"] = pd.to_datetime(self.training_data["Timestamp_Local"])
    ...
    def load_test_data(self, version: str = "v0.2") -> pd.DataFrame:
        ...
        self.test_data["Timestamp_Local"] = pd.to_datetime(self.test_data["Timestamp_Local"])
    ...
    def split_train_validation(self, data: pd.DataFrame, validation_size: float = 0.2, time_based: bool = True) -> Tuple[pd.DataFrame, pd.DataFrame]:
        if time_based:
            ...
            train_df, val_df = train_test_split(data, test_size=validation_size, random_state=42)
    ...
    def get_data_statistics(self, data: pd.DataFrame) -> dict:
        stats = {
            ...
            "date_range": (data["Timestamp_Local"].min(), data["Timestamp_Local"].max()),
